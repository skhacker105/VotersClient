...
export class DiscussionCarouselComponent implements OnInit, OnDestroy {
  ...
  gotoVoteDiscussion(voteType: IVoteType) {
    if (!this.discussion) return;
    this.router.navigateByUrl(
      `/home?id=${this.discussion._id}&ui_id=${voteType.ui_id}`
    );
  ...
  getVoteCategoryCount(votetype?: IVoteType): number {
    ...
    const existingCategory = this.discussion.voteCategories.find(
      (cat) => cat.category.ui_id === votetype.ui_id
    );
  ...
  handleChangeState(newState: IDiscussionState) {
    if (!this.discussion) return;
    this.discussion
      .confirmForStateChange(newState)
      .then((result) => {
  ...
  changeState(newState: IDiscussionState) {
    ...
    this.discussionService
      .updateState(this.discussion._id, newState)
      .pipe(takeUntil(this.isComponentIsActive))
      .subscribe({
